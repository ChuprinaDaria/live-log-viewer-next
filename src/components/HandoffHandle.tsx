...
interface Props {
  ...
  onHandoff: () => void;
  /** «Перенести» (Task 7): sessionmem writes a portable brief and starts a
      fresh draft with it. Absent wherever the caller has not wired it (map
      mode, callers that predate the feature) — the transfer button then just
      does not render, and the handoff pill is unaffected. */
  onTransfer?: () => void;
 ...
 */
export function HandoffHandle({ file, onHandoff, onTransfer, inline = false }: Props) {
  const { t } = useLocale();
  ...
  return (
    <div data-scheme-ui className="absolute -bottom-11 left-2 z-[2] flex items-center gap-1.5">
      <div className="group">
        <button
          type="button"
          aria-label={t("handoff.aria")}
      ...
      {onTransfer ? (
        <button
          type="button"
          data-transfer
          aria-label={t("transfer.label")}
          title={t("transfer.label")}
          onClick={onTransfer}
          className="flex h-7 w-7 items-center justify-center rounded-full border border-border bg-card text-muted shadow-1 hover:border-accent/50 hover:text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40"
        >
          <ArrowRightLeft className="h-3.5 w-3.5 shrink-0" aria-hidden />
        </button>
      ) : null}
